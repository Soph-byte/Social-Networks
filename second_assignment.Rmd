...
The positive predictive value (precision) is high at 93.11%, meaning when the model predicts a link exists, it is usually correct.

The negative predictive value is around 59.90%, reflecting that predictions of “no link” are less reliable.

Overall, this higher threshold makes the model more conservative about predicting links, reducing false positives but increasing false negatives. It’s a trade-off favoring precision over recall, which might be suitable if false positives are more costly than false negatives in the application context.
...
Precision = 93.1 % at the optimum threshold (≈ 0.787) Means: 93 % of the predicted positive links were actually real connections Very good prediction accuracy for existing edges

```{r}
summary(glm_link)
```

Most important heuristic: Adamic-Adar (sim_aa) Highest coefficient in the GLM (β ≈ 2.91) Very high z-value (18.1) → strong effect Extremely significant (p \< 2e-16) Provides the largest contribution to predictive power -\> considers common neighbors, weighted according to their information content

sim_jacc: Not significant (p = 0.203) Not a reliable predictor in this data set sim_pref: Significant (p ≈ 5e-07) But very small effect (coefficient ≈ 0.000057)\
**Adamic-Adar is therefore the most important heuristic in this model, both statistically and theoretically.**

## 5. Comment on potential ways to improve the link prediction

-   Using more advanced similarity measures that are better suited for bipartite networks

-   Adding node attributes (f.e. user activity, ratings) to include more information in the model

-   Trying other classification models like Random Forest or Support Vector Machines

-   Handling class imbalance better (f.e. by oversampling positive cases or using different thresholds)

-   Tuning hyperparameters or use ensemble methods for better performance

-   Considering temporal aspects if the network has timestamps (f.e. newer links may be more predictable)

# Conclusion

In this assignment, we worked on the problem of link prediction using a real-world bipartite social network. We simulated missing links by removing a random subset of edges and then used three standard similarity heuristics—Jaccard, Adamic-Adar, and Preferential Attachment—to calculate scores for each link. These heuristics helped us build a logistic regression model to predict whether a link exists or not. The Adamic-Adar heuristic turned out to be the most useful, showing the strongest effect in the model. Preferential Attachment also contributed, but with a smaller impact. Jaccard similarity did not appear to be statistically significant in this case. While the model achieved a high precision of over 93% at the optimal threshold, recall was relatively low, meaning it missed many of the real links. This suggests that the model was conservative and biased toward predicting non-links, which should be taken into account when evaluating performance. Additionally, the bipartite nature of the network may have influenced the effectiveness of the heuristics, especially those relying on neighborhood overlap. Since nodes are of two different types, direct neighbors are not of the same class, which could affect similarity measures and their predictive power. To improve the results, future work could involve testing other heuristics better suited for bipartite graphs, using node attributes, or trying different types of models. Overall, the project gave us a better understanding of how structural properties of networks can be used for link prediction and where the limitations lie.
